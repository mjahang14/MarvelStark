...
function initMap() {
   ...
   "properties": {
	...
	nfz.geometry.coordinates[0].push([LL.lng,LL.lat]);
	});
	var start_lat = nfz.geometry.coordinates[0][0][0];
	var start_lng = nfz.geometry.coordinates[0][0][1];
	nfz.geometry.coordinates[0].push([start_lat,start_lng]);

	var xhr = new XMLHttpRequest();
	xhr.open('POST','/api/populateNFZ/',true);
	xhr.setRequestHeader("Content-Type","application/json");
	xhr.send(JSON.stringify(nfz));
...
xhr.onreadystatechange = function()
	{
		if (xhr.readyState == XMLHttpRequest.DONE)
		{
			...
			console.log("Print Response: " +nfz);
