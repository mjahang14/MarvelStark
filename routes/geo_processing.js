...
getRoute_Grid : function (src, dest, isGrid, callback) {
			...
			if (roundDistance <= 1) {
				...
				gridWidth = 750
			} else if (roundDistance > 25 && roundDistance <= 35) {
				gridWidth = 1500
			} else if (roundDistance > 35 && roundDistance <= 45) {
				gridWidth = 2000
			} else {
				gridWidth = 5000
			}
			...
			mongo_handle.getDocumentsRouting("geo_nfz_layer_drawing", queryParams, searchParams, function (err, listing) {
					...
					var grid_counter = lowVal
					var srcIndex, dstIndex = lowVal;
					...
					squareGrid.features.forEach(function (singleFeatureGrid) {
							...
							if (ptsWithin.features.length > lowVal) {
