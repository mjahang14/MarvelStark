...
function initMap() {
  ...
  searchBox.addListener('places_changed', function() {
	...
	places.forEach(function(place) {
		...
		if((routeres)&&(routeret))
		{
			console.log("route removal",routeres);
...
var templng;
    ...
    if (xhr.readyState == XMLHttpRequest.DONE)
		{
			var nfz = JSON.parse(xhr.responseText);
			//console.log("Print Response: " + JSON.stringify(nfz));
			//console.log(nfz.features.length);
			var temp_geo;

			nfz.features.forEach(function(item,index){
...
function grid()
{
		...
		var temp_grid={
            ...
            temp_grid.features[0].geometry.coordinates.push([source_global.lng,source_global.lat]);
			temp_grid.features[1].geometry.coordinates.push([dest_global.lng,dest_global.lat]);
			//console.log(temp_grid);

			var xhr = new XMLHttpRequest();
			xhr.open('POST','/api/getGrid/',true);
			xhr.setRequestHeader("Content-Type","application/json; charset=UTF-8");
			xhr.send(JSON.stringify(temp_grid));
...
k.features[1].geometry.coordinates.push([dest_global.lng,dest_global.lat]);
	...
	xhr.onreadystatechange = function()
	{
		if (xhr.readyState == XMLHttpRequest.DONE)
			{
			routeres = JSON.parse(xhr.responseText);
			console.log("route response",routeres);
			if(routeres.properties.result == false)
			{
			document.getElementById("modal").click();
			document.getElementById("noroute").innerHTML = routeres.properties.info;
			console.log("route does not exist");
			console.log(routeres);
			return;
			}
			routeret = map.data.addGeoJson(routeres);
			map.data.setStyle(function(feature)
			{
				var prop = feature.getProperty("routeFlag");
				var color;
